...
/**
 * @en An authentication component that provides UI for user sign-in and registration.
 *     It supports email/password and Google Sign-In methods.
 * @de Eine Authentifizierungskomponente, die eine Benutzeroberfläche für die Benutzeranmeldung und -registrierung bereitstellt.
 *     Sie unterstützt E-Mail/Passwort und Google Sign-In Methoden.
...
const Auth: React.FC = () => {
    ...
    /**
     * @en Handles the form submission for email/password authentication.
     * @de Behandelt das Absenden des Formulars für die E-Mail/Passwort-Authentifizierung.
    ...
    /**
     * @en Handles the Google Sign-In process.
     * @de Behandelt den Google Sign-In-Prozess.
     */
        ...
        <div className="flex items-center justify-center h-screen w-screen bg-gray-900">
            <div className="w-full max-w-md p-8 space-y-6 bg-gray-800 rounded-lg shadow-lg border border-gray-700">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        <label htmlFor="password"className="text-sm font-medium text-gray-300 block">Password / Passwort</label>
                        <input
                            id="password"
                            type="password"
                            value={password}
                    ...
                    <div>
                        <button type="submit" disabled={loading} className="w-full px-4 py-2 font-bold text-white bg-blue-600 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:bg-gray-500">
                            {loading ? 'Loading... / Lade...' : (isLogin ? 'Login / Anmelden' : 'Register / Registrieren')}
                        </button>
                    </div>
                </form>

                <div className="relative flex items-center justify-center">
                    <span className="absolute px-3 bg-gray-800 text-gray-500">Or / Oder</span>
                ...
                <p className="text-sm text-center text-gray-400">
                    {isLogin ? "Don't have an account? / Noch kein Konto?" : 'Already have an account? / Bereits ein Konto?'}
                    <button onClick={() => setIsLogin(!isLogin)} className="font-medium text-blue-400 hover:underline ml-1">
                        {isLogin ? 'Register / Registrieren' : 'Login / Anmelden'}
                    </button>
                </p>
            </div>
        </div>
